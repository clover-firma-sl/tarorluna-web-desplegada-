...
export function SocialPage() {
  return <SiteShell><main className="inner-page social-page"><section className="section-heading"><p className="eyebrow">Síguenos</p><h1>Redes sociales de Tarot Luna</h1><p>Accede directamente a los perfiles oficiales de Tarot Luna.</p></section><section className="social-cards"><a href="https://www.tiktok.com/@tarotlunaaa4" target="_blank" rel="noopener noreferrer"><span className="social-network-icon">♪</span><div><small>TikTok</small><strong>@tarotlunaaa4</strong></div><ExternalLink /></a><a href="https://www.instagram.com/tarot_lunaaaa/" target="_blank" rel="noopener noreferrer"><span className="social-network-icon"><AtSign /></span><div><small>Instagram</small><strong>@tarot_lunaaaa</strong></div><ExternalLink /></a></section></main></SiteShell>;
}

export function CategoryPage({ categoryKey }: { categoryKey: CategoryKey }) {
  const category = categories[categoryKey]; const Icon = category.icon;
  return <SiteShell><main className="inner-page"><section className="category-hero"><span className="large-icon"><Icon /></span><p className="eyebrow">{category.eyebrow}</p><h1>{category.title}</h1><p>{category.description}</p></section><section className="consultation-panel"><div><p className="eyebrow">Tu consulta puede incluir</p><h2>Una lectura adaptada a tu situación</h2><ul>{category.topics.map((topic) => <li key={topic}><Check />{topic}</li>)}</ul></div><div className="duration-card"><span>Tres formatos disponibles</span><strong>10, 30 o 60 minutos</strong><p>Elige la duración que mejor se adapte a lo que necesitas.</p><Link className="primary-button" href={`/reservar?consulta=${categoryKey}`}>Reservar esta consulta <ArrowRight /></Link></div></section></main></SiteShell>;
}
